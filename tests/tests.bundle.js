...
_get__('testsContext').keys().forEach(_get__('testsContext'));
...
exports.parseTextAndBuildHashtags = parseTextAndBuildHashtags;
exports.truncateDOM = truncateDOM;
/**
 ...
 */
function parseTextAndBuildHashtags(text) {

  if (!text) {
    return '';
  }

  var parsedText = void 0;

  // replace hashtags and mentions
  parsedText = text.replace(/#(\w+)/g, '<a href="https://twitter.com/hashtag/$1?src=hash" target="_blank">#$1</a>').replace(/@(\w+)/g, '<a href="https://twitter.com/$1" target="_blank">@$1</a>');
 ...
 */
function truncateDOM(parent, child, readMoreText) {
  ...
  var length = readMoreText.length;
  var cur = 0;

  // let's remove more words, until we get to a point where we can fit '   ...read more'
  while (cur < length) {
    var _text = child.innerHTML;
    var match = _text.match(/\W*\s(\S)*$/);
    cur += match[0].length;
    _text = _text.replace(/\W*\s(\S)*$/, '');
    child.innerHTML = _text;
  }
}
...
var _truncateDom = __webpack_require__(3);

var _truncateDom2 = _interopRequireDefault(_truncateDom);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

describe('dom-utils: ', function () {

  it('truncates a paragraph until it does not exceed parent\'s height', function (done) {

    var document = window.document;
    var main = document.getElementById('mocha');
    main.innerHTML = _get__('truncateDOMFixture');
    var parent = window.document.querySelector('#parent');
    var child = window.document.querySelector('#child');

    _get__('truncateDOM')(parent, child);
...
function _get_original__(variableName) {
  switch (variableName) {
    case 'truncateDOMFixture':
      return _truncateDom2.default;

    case 'truncateDOM':
      return _index.truncateDOM;
  }
